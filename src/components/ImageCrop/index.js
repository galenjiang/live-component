...
export default class ReactCrop extends Component {
  ...
  static propTypes = {
    ...
    onCompleteAutoToBase64: PropTypes.func,
  ...
  componentDidMount() {
    ...
    if (this.imageRef.complete || this.imageRef.readyState) {
      if (this.imageRef.naturalWidth === 0) {
        ...
        const emptyGif = 'data:image/gifbase64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=='
  ...
  onDocMouseTouchEnd() {
    ...
    if (this.mouseDownOnCrop) {
      ...
      if (this.props.onCompleteAutoToBase64) {
        ...
        let cropAfterLoad = (loadedImg) => {
          ...
          this.props.onCompleteAutoToBase64(canvas.toDataURL(), cropWidth, cropHeight)
        }
        cropAfterLoad = cropAfterLoad.bind(this)
        loadImage(this.imageRef.src, cropAfterLoad)
      }
  ...
  nextCropState(crop) {
    const nextCrop = Object.assign({}, ReactCrop.defaultCrop, crop)
    this.cropInvalid = this.isCropInvalid(nextCrop)
    return nextCrop
  }
  ...
  ensureAspectDimensions(cropObj, imageEl) {
    ...
    const crop = Object.assign({}, cropObj)
